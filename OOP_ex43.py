...
class Corridor (Scene):
    def enter(self):
        print("""
        Let see how smart you're , you need need to take some decision to defeat the aliens who have invaded the ship 
        and reach the planet below. So how you want to start this journey (you just need to enter the number and the press ENTER).
        1) Find a gun to figth against them.
        ...
        4) Do nothing, expect to some to save you.
        """)
        Option = input("> ")
        if Option == '1':
            print("""
...
class LeaserWeapon (Scene):
    def enter(self):
        print("""
        You have reached the armory, there are plenty of weapons, but you can only carry 1.
        Choose wisely because there are advantages and disavantages for each one:
        1) Choose the always reliable laser handgun, this weapon will allow you to run fast but the laser charge wont last to much.
        2) Choose the bazooka, you will only have one shoot, but with a huge destructive power.
        3) Choose asault rifle, a good balance between wight and power capacity, but the normal bullet will kill the aliens?
        """)
        Option = input("> ")
        if Option == '1':
            print("""
            ...
            print("""
            You only have one, shoot... you don't waste time and trigger the missile to the aliens. 
            You need to enter a number between 1-5 if you guess the number you missile will kill the alines,
            other wise you will go to the bridge without weapon.
            """)
